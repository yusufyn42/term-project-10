...
def create_registration(registrations: list,
    ...
    new_id = f"R{len(registrations) + 1}"
    ...
    registration = {
        ...
        "amount": amount,
        "confirmation_code": f"{event_id}-{new_id}",
        # optional: bir registration hangi sessionlara girebilir
...
def promote_waitlist(registrations: list, event_id: str) -> dict | None:
    for reg in registrations:
        if reg.get("event_id") == event_id and reg.get("status") == "waitlisted":
            new_seat = _count_confirmed(registrations, event_id) + 1
            reg["status"] = "confirmed"
            reg["seat_number"] = new_seat
